...
//        }
    ...
    {
        $customerUserId = (!empty($param['customer_user'])) ? $param['customer_user'] : '';
        $res = [];
        ...
        $limit = isset($param['limit']) ? intval($param['limit']) : 12;
        $page = isset($param['page']) && $param['page'] ? intval($param['page']) : 1;
        $offset = $limit * ($page - 1);

        $valid_search_columns = ['id', 'status', 'user_email', 'user_nicename', 'first_name', 'billing_phone'];
        $search_column = isset($param['type']) && in_array($param['type'], $valid_search_columns) ? $param['type'] : '';
        $search_value = isset($param['name']) ? trim($param['name']) : '';
        ...
        if ($search_column === 'billing_phone') {
            ...
            $count_args['search'] = '';
            $count_args['search_columns'] = [];
        }

        if ($search_column === 'status') {
            // WooCommerce order statuses use the "wc-" prefix, e.g., 'wc-processing'
            $count_args['status'] = $search_value;

            // Clear 'search' and 'search_columns' since we're focusing on status
            $count_args['search'] = '';
            $count_args['search_columns'] = [];
        }
        ...
        if ($search_column === 'status') {
            $search_args['status'] = $count_args['status'];
        }

        if ($search_column === 'id') {
            $search_args['include'] = $count_args['include'];
        }

        $query = new WC_Order_Query($search_args);
...
//      $this->add_address_book($param);
    ...
    public function getuserdetail($param = array())
    {
        ...
        if (!$phone) {
            $phone = get_user_meta($param['id'], 'phone', true);
        }
    ...
    public function getuserlist($param = array())
    {
        ...
        $valid_search_columns = ['id', 'user_email', 'user_nicename', 'first_name', 'billing_phone'];
        $search_column = isset($param['type']) && in_array($param['type'], $valid_search_columns) ? $param['type'] : '';
        $search_value = isset($param['name_mail_phone']) ? trim($param['name_mail_phone']) : '';
        ...
        if (in_array($search_column, ['user_nicename', 'user_email'])) {
            $count_args['search_columns'] = [$search_column];
        }

        if ($search_column === 'id') {
            $count_args['include'] = [$search_value];
        ...
        $totalUsers = $count_query->get_total();
